...
$qty = $this->adjust_quantity($qty, $products_id, 'shopping_cart');
  ...
  function remove($products_id) {
    ...
    $ec_analytics = new ec_analytics();
    $ec_analytics->generate_info('NOTIFIER_CART_REMOVE_END');
  ...
  function calculate() {
    ...
    while (list($products_id, ) = each($this->contents)) {
      ...
      if ($product = $db->Execute($product_query)) {
		...
		/*BOF Rajeev GreyBox added this to handle the facebook like discount*/
		if($_SESSION['customer_id']) {
			$display_facebook_price=0;
			$product_facebook_discount = $db->Execute("select products_id from facebook_users_products where customer_id = " . $_SESSION['customer_id'] . " and products_id = '" . (int)$prid . "'");
			if ($product_facebook_discount->RecordCount() > 0) {
				$products_price = $products_price-($products_price * 0.25);
			}
		}
		/*EOF Rajeev GreyBox added this to handle the facebook like discount*/
...
//  echo 'I SEE ' . $this->total . ' vs ' . $this->free_shipping_price . ' items: ' . $this->free_shipping_item. '<br>';
  ...
  function get_products($check_for_valid_cart = false) { 
    ...
    reset($this->contents); 
    while (list($products_id, ) = each($this->contents)) {
		/*Rajeev GreyBox added two fields for the OSDS automation p.products_is_osds and p.products_osds_name*/
      $products_query = "select p.products_id, p.products_is_osds, p.products_osds_name, p.master_categories_id, p.products_status, pd.products_name, p.products_model, p.products_image,
...
// validate cart contents for checkout
        ...
        //                                    'quantity' => $this->contents[$products_id]['qty'],
                                  ...
                                  'products_is_osds' => $products->fields['products_is_osds'],
                                  'products_osds_name' => $products->fields['products_osds_name'],
...
//echo 'I SEE actionMultipleAddProduct: ' . $prodId . '<br>';
          ...
          if (($add_max == 1 and $cart_qty == 1)) {
            ...
            //Rajeev GreyBox added the functionality that lets add multiple products with attributes.
            //Please bear in mind, zen-cart doesn't support this by default and we needed this for our promotional pages and emails
            //where we are giving one button to add multiple e-books at once, since e-books can not be added without the attribute
            //this change was mandatory.
            //Following one statement was commented and
...
for ($i=0, $n=sizeof($products); $i<$n; $i++) {
  ...
  $productsImage = (IMAGE_SHOPPING_CART_STATUS == 1 ? zen_image(HTTP_SERVER.DIR_WS_CATALOG.DIR_WS_IMAGES . $products[$i]['image'], $products[$i]['name'], IMAGE_SHOPPING_CART_WIDTH, IMAGE_SHOPPING_CART_HEIGHT) : '');
...
//  $showFixedQuantityAmount = $products[$i]['quantity'] . zen_draw_hidden_field('cart_quantity[]', 1);
  ...
  $quantityField = $products[$i]['quantity'];
...
// zenbundles: 1 of 2 start
  //$productArray[$i] = array('attributeHiddenField'=>$attributeHiddenField,							  
  $productArray[] = array('attributeHiddenField'=>$attributeHiddenField,
// zenbundles: 1 of 2 end
...
// zenbundles: 2 of 2 start
                            'attributes'=>$attrArray,
							'inBundle'=>false,
							'isBundle'=>( zen_get_info_page($products[$i]['id']) == 'product_bundle_info' ? true : false ));
  
  if (zen_get_info_page($products[$i]['id']) == 'product_bundle_info') {
	  // get product in bundle ... loop thru and add to array
	  // fixed quantity and no delete
	  if (isset($products[$i]['products'])) {
		  foreach($products[$i]['products'] as $id => $product) {
			  	$product_id = key($product);
			  
				if (STOCK_CHECK == 'true') {
					
					$flagStockCheck = zen_check_stock($product_id, $product[$product_id]['quantity'] * $products[$i]['quantity']);
					if ($flagStockCheck == true) {
					  $flagAnyOutOfStock = true;
					}
					$stockAvailable = zen_get_products_stock($product[$product_id]['id']);		
				  }			  
				  
				$showFixedQuantity = true;
				//$showFixedQuantityAmount =  $product[$product_id]['quantity'] * $products[$i]['quantity'];			  
				$showFixedQuantityAmount =  $product[$product_id]['quantity'];			  
				$showMinUnits = zen_get_products_quantity_min_units_display($product_id);
				$linkProductsImage = zen_href_link(zen_get_info_page($product_id), 'products_id=' . $product_id);
				$linkProductsName = zen_href_link(zen_get_info_page($product_id), 'products_id=' . $product_id);
				$productsName  = $product[$product_id]['name'];
				$productsImage = (IMAGE_SHOPPING_CART_STATUS == 1 ? zen_image(DIR_WS_IMAGES . $product[$product_id]['image'], $product[$product_id]['name'], IMAGE_SHOPPING_CART_WIDTH, IMAGE_SHOPPING_CART_HEIGHT) : '');
				$attrArray = false;
				...
				$bundleProduct = $db->Execute("SELECT IF(b.products_price,b.products_price,p.products_price) products_price, 
												 ...
												 WHERE b.bundles_id = ".(int)$products[$i]['id']." 
				...
				if (!$bundleProduct->EOF) { 
                    ...
                    if (is_array($product[$product_id]['attr'])) {	
    					foreach($product[$product_id]['attr'] as $option => $value) {
    						...
    						if ($bundleOptionValue['name'] != '') {
    							$attr_value = stripslashes($bundleOptionValue['name']);
    						} else {
    							$attr_value = $attributes_values->fields['products_options_values_name'];
    						}
    						$attrArray[$option]['products_options_name'] = $attributes_values->fields['products_options_name'];
    					    ...
    					    if ( STOCK_CHECK == 'true' ) {
    							...
    							if(is_array($product[$product_id]['attr'])){
    								$attributes = $product[$product_id]['attr'];
    							} else {
    								$attributes = '';
    							}
    							...
    							$stock_check = zen_check_stock($product_id, $product[$product_id]['quantity'] * $products[$i]['quantity'],$attributes);
    					
    							if (zen_not_null($stock_check))
    							{
    								$flagAnyOutOfStock = true;
    								$flagStockCheck = $stock_check;
    							    $stockAvailable = zen_get_products_stock($product_id, $attributes);		
    							}
    						}						
					...
					$productArray[] = array('attributeHiddenField'=>'',							  
											'flagStockCheck'=>$flagStockCheck,
											'flagShowFixedQuantity'=>$showFixedQuantity,
											'linkProductsImage'=>$linkProductsImage,
											'linkProductsName'=>$linkProductsName,
											...
											'id'=>$products[$i]['id'],
											'attributes'=>$attrArray,
											'inBundle'=>$products[$i]['id'],
											'isBundle'=>false);
		  		}
			}
	  }
  }
// zenbundles: 2 of 2 end

} // end FOR loop
        
       //  send cart content to Customer.io      
           $cart_content = '<table  border="0" width="100%" cellspacing="0" cellpadding="0" id="cartContentsDisplay">
     <tr class="tableHeading">
        <th scope="col" id="scQuantityHeading">'.TABLE_HEADING_QUANTITY.'</th>
        <th scope="col" id="scProductsHeading">'.TABLE_HEADING_PRODUCTS.'</th>
        <th scope="col" id="scUnitHeading">Price</th>
        <th scope="col" id="scTotalHeading">'.TABLE_HEADING_TOTAL.'</th>
     </tr>';
        ...
        <tr>
          ...
          if ($product['flagShowFixedQuantity']) {
            $cart_content .= $product['showFixedQuantityAmount'] . '<br /><span class="alert bold">' . $product['flagStockCheck'] . '</span><br />' . $product['showMinUnits'];
          } else {
            $cart_content .= $product['quantityField'] . '<br /><span class="alert bold">' . $product['flagStockCheck'] . '</span><br /><br />' . $product['showMinUnits'];
          }
          $cart_content .= '</td>';  //   cartQuantity
            
            $cart_content .= '<td class="cartProductDisplay">
<a href="'.$product['linkProductsName'].'"><span id="cartImage" class="back"><span class="image">'.$product['productsImage'].'</span></span><span id="cartProdTitle">'.$product['productsName'] . '<span class="alert bold">' . $product['flagStockCheck'] . '</span>'.'</span></a>
<br class="clearBoth" />'.$product['attributeHiddenField'];

          if (isset($product['attributes']) && is_array($product['attributes'])) {
            $cart_content .= '<div class="cartAttribsList">';
            $cart_content .= '<ul>';
            ...
            foreach ($product['attributes'] as $option => $value) {
                ...
                $cart_content .= '<li>'.$value['products_options_name'] . ':' . nl2br($value['products_options_values_name']).'</li>';
           ...
           $cart_content .= '</td>';  //  cartProductDisplay
           $cart_content .= '<td class="cartUnitDisplay price">'.$product['productsPriceEach'].'</td>
            <td class="cartTotalDisplay">'.$product['productsPrice'].'</td>
           </tr>';
       }   //   foreach ($productArray as $product) {
