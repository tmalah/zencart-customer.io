 ...
 * @version $Id: order.php 109 2010-04-28 05:47:55Z numinix $
 */
/**
 * order class
 *
...
class order extends base {
  ...
  function query($order_id) {
    ...
    $order_query = "select customers_id, customers_name, customers_company,
                         ...
                         date_purchased, orders_status, last_modified, order_total, order_tax, ip_address, shipping_weight_total
                        from " . TABLE_ORDERS . "
    ...
    $this->info = array('currency' => $order->fields['currency'],
                        ...
                        'shipping_weight_total' => $order->fields['shipping_weight_total']
                        );
    ...
    $orders_products_query = "select orders_products_id, products_id, products_name,
                                 ...
                                 products_discount_type_from, products_is_osds, products_osds_name
    ...
    while (!$orders_products->EOF) {
      ...
      $this->products[$index] = array('qty' => $new_qty,
                                      ...
                                      'products_is_osds' => $orders_products->fields['products_is_osds'],
                                      'products_osds_name' => $orders_products->fields['products_osds_name']);
                                      
// zenbundles: 1 of 4 start
	  $this->notify('NOTIFY_ORDER_QUERY_ADD_PRODUCT_LIST', array('index'=>$index,'orders_products'=>$orders_products->fields));
// zenbundles: 1 of 4 end
    ...
    if (FEC_GIFT_WRAPPING_SWITCH == 'true') {
      // Gift wrap
      $wrap_query = "select orders_products_id, wrap from " . 
                           TABLE_ORDERS_GIFTWRAP . 
                           " where orders_id = '" . (int)$order_id . "'";

      $wraplist = $db->Execute($wrap_query);

      while (!$wraplist->EOF) {
        $this->wrapped [] = array('orders_products_id' => $wraplist->fields['orders_products_id'],
                                'wrapped' => $wraplist->fields['wrapped']);
        $wraplist->MoveNext();
      }
    }
  }
  ...
  function cart() {
    ...
    global $db, $currencies, $orderWatcher; // zenbundles: 2 of 4
    ...
    if (isset($_SESSION['cc_id'])) {
      $coupon_code_query = "select coupon_code, is_refiral
                              from " . TABLE_COUPONS . "
                              where coupon_id = '" . (int)$_SESSION['cc_id'] . "'";

      $coupon_code = $db->Execute($coupon_code_query);

        if ($coupon_code->fields['is_refiral'] == '1') {
            $_SESSION['is_refiral'] = '1';
        }
    }
...
*/
    ...
    for ($i=0, $n=sizeof($products); $i<$n; $i++) {
      ...
      //  added by Taras to store OSDS info for products BOF
      if (!isset($products[$i]['products_is_osds'])) {
        $get_osds_query = "SELECT products_is_osds, products_osds_name
                            FROM ".TABLE_PRODUCTS."
                            WHERE products_id = ".$products[$i]['id'];
        $get_osds = $db->execute($get_osds_query);
        $products[$i]['products_is_osds'] = $get_osds->fields['products_is_osds'];
        $products[$i]['products_osds_name'] = $get_osds->fields['products_osds_name'];
      }
      //  added by Taras to store OSDS info for products EOF
      ...
      $this->products[$index] = array('qty' => $products[$i]['quantity'],
                                      ...
                                      'id' => $products[$i]['id'],
                                      /*** added by Taras to check is in bundle   ***/
                                      'bundle_id' => isset($products[$i]['bundle_id']) ? (int)$products[$i]['bundle_id'] : '',
                                      /*Rajeev GreyBox added two fields for the OSDS automation products_is_osds and products_osds_name*/
                                      'products_is_osds' => $products[$i]['products_is_osds'],
                                      'products_osds_name' => $products[$i]['products_osds_name'],
                                      'rowClass' => $rowClass);

      //if ($products[$i]['attributes']) {
       
       //  added by Taras: trigger to store attributes for combo products BOF
      $this->notify('NOTIFY_ORDER_CART_ADD_PRODUCT_LIST', array('index'=>$index, 'products'=>$products[$i]));
      //  added by Taras: trigger to store attributes for combo products EOF 
        
// zenbundles: 3 of 4 start
      //if ($products[$i]['attributes']) {
	  if ($products[$i]['attributes'] && $orderWatcher->processAttributes()) {
// zenbundles: 3 of 4 end
...
/*BOF, Rajeev greybox, check if it is a pure e-order then set the status to Digital Download*/
	if($this->check_if_skip_order()) { 
		$this->info['order_status'] = 6;
	} 
/*EOF, Rajeev greybox, check if it is a pure e-order then set the status to Digital Download*/
    ...
    $sql_data_array = array('customers_id' => $_SESSION['customer_id'],
                            ...
                            'ip_address' => $_SESSION['customers_ip_address'] . ' - ' . $_SERVER['REMOTE_ADDR'],
                            'shipping_weight_total' => $_SESSION['cart']->weight, //Rajeev added this to store the Shipping weight in the order table
                            'ccavenue_orders_id' => $_SESSION['ccavenue_orders_id'] //Rajeev added this to store CCAvenue Order ID
                            );
    /*** BOF Rajeev, now remove the records from the temp order tables ***/
	//Get orders_id so that all the matching records from 5 temp order tables can be deleted
	$get_order_id_query = "select orders_id
		from " . TABLE_TEMP_ORDERS . "
		where ccavenue_orders_id = '" . $_SESSION['ccavenue_orders_id'] . "'";
	$get_order_id = $db->Execute($get_order_id_query);

	//Delete the order details from TABLE_TEMP_ORDERS_PRODUCTS_DOWNLOAD
	$delete_temp_orders_sql = "DELETE FROM " . TABLE_TEMP_ORDERS_PRODUCTS_DOWNLOAD . "
		  WHERE  orders_id = '" . $get_order_id->fields['orders_id'] . "'";
	$db->Execute($delete_temp_orders_sql);

	//Delete the order details from TABLE_TEMP_ORDERS_PRODUCTS_ATTRIBUTES
	$delete_temp_orders_sql = "DELETE FROM " . TABLE_TEMP_ORDERS_PRODUCTS_ATTRIBUTES . "
		  WHERE  orders_id = '" . $get_order_id->fields['orders_id'] . "'";
	$db->Execute($delete_temp_orders_sql);

	//Delete the order details from TABLE_TEMP_ORDERS_PRODUCTS
	$delete_temp_orders_sql = "DELETE FROM " . TABLE_TEMP_ORDERS_PRODUCTS . "
		  WHERE  orders_id = '" . $get_order_id->fields['orders_id'] . "'";
	$db->Execute($delete_temp_orders_sql);

	//Delete the order details from TABLE_TEMP_ORDERS_TOTAL
	$delete_temp_orders_sql = "DELETE FROM " . TABLE_TEMP_ORDERS_TOTAL . "
		  WHERE  orders_id = '" . $get_order_id->fields['orders_id'] . "'";
	$db->Execute($delete_temp_orders_sql);

	//Finally, delete the order now
	$delete_temp_orders_sql = "DELETE FROM " . TABLE_TEMP_ORDERS . "
		  WHERE  orders_id = '" . $get_order_id->fields['orders_id'] . "'";
	$db->Execute($delete_temp_orders_sql);
    /*** EOF Rajeev, now remove the records from the temp order tables ***/
    ...
    for ($i=0, $n=sizeof($this->products); $i<$n; $i++) {
      ...
      if (STOCK_LIMITED == 'true') {
        if (DOWNLOAD_ENABLED == 'true') {
          $stock_query_raw = "select p.products_quantity, pad.products_attributes_filename, p.product_is_always_free_shipping, products_is_osds, products_osds_name
      ...
      if (isset($_SESSION['cc_id']) && $_SESSION['cc_id'] != '') { 
        if (is_product_valid(zen_get_prid($this->products[$i]['id']), $_SESSION['cc_id'])) {
            $coupon_sql = "select coupon_id, coupon_amount, coupon_code, coupon_type
                            from " . TABLE_COUPONS . "
                            where coupon_id = '".$_SESSION['cc_id']."'
                            and coupon_active='Y'";
            $coupon_info = $db->execute($coupon_sql);
            if ($coupon_info->RecordCount() > 0) {
                $coupon_code = $coupon_info->fields['coupon_code'];
                if ($coupon_info->fields['coupon_type'] == 'P') {
                    $discount_amount = round($this->products[$i]['price']*($coupon_info->fields['coupon_amount']/100), 2);
                } elseif ($coupon->fields['coupon_type'] == 'F') {
                    //$discount_amount = $coupon->fields['coupon_amount'] * ($order_total>0);
                    //$od_amount['type'] = 'F';
      ...
      //  get price in bundle
      $bundle_price = 0;
      if (isset($this->products[$i]['bundle_id']) && $this->products[$i]['bundle_id'] != '') {
        
        $get_bundle_price_sql = "SELECT products_price FROM ".TABLE_BUNDLES."
                                 WHERE bundles_id = '".$this->products[$i]['bundle_id']."'
                                 AND products_id = '".zen_get_prid($this->products[$i]['id'])."'";
        $get_bundle_price = $db->execute($get_bundle_price_sql);
        
        if ($get_bundle_price->RecordCount() > 0) {
            $bundle_price = $get_bundle_price->fields['products_price'];
        }        
      }
      ...
      $sql_data_array = array('orders_id' => $zf_insert_id,
                              ...
                              'products_is_osds' => $this->products[$i]['products_is_osds'],
                              'products_osds_name' => $this->products[$i]['products_osds_name']);
      zen_db_perform(TABLE_ORDERS_PRODUCTS, $sql_data_array);

      $order_products_id = $db->Insert_ID();
      //echo '<pre>';print_r($this->products[$i]);echo '</pre>LordShiva';exit;
/*Rajeev GreyBox, OSDS automation Starts*/
	  if($this->products[$i]['products_is_osds']=="Y") {
		  $curl_dap_data = array(
				'web_accept' => 'buynow',
				'first_name' => $this->customer['firstname'],//'Rajeev',
				'last_name' => $this->customer['lastname'],//'Sharma',
				'payer_email' => $this->customer['email_address'],//'rajeev.sharma@msn.com',
				'address_city' => $this->customer['city'],//'New Delhi',
				'address_state' => $this->customer['state'],//'Delhi',
				'address_zip' => $this->customer['postcode'],//'110018',
				'address_country' => $this->customer['country']['title'],//'India',
				'mc_gross' => $this->products[$i]['final_price'],//'499',
				'mc_currency' => 'INR',
				'payment_status' => 'Completed',
				'item_name' => $this->products[$i]['products_osds_name'],//$this->products[$i]['products_osds_name'],//'NSO Level 2 - Class 3',
				'secret_key' => 'mtgbooks'
			);
			$http_code=curl_dap($curl_dap_data);
			if($http_code!=200){
				//do something about it.
			}
			//echo '<pre>';print_r($curl_dap_data);echo '</pre>LordShiva'.$http_code;exit;
	  }
      
      // added by Taras to check products in bundle for OSDS BOF
      /*if (isBundle($this->products[$i]['products_id'])) {
        $bundle_products_sql = "SELECT b.products_id, b.products_price, b.options, b.products_quantity, p.products_is_osds, p.products_osds_name
                                FROM ".TABLE_PRODUCTS." p, ".TABLE_BUNDLES." b
                                WHERE p.products_id = b.products_id
                                AND b.bundles_id = ".$this->products[$i]['products_id'];
        $bundle_products = $db->execute($bundle_products_sql);
        
        while (!$bundle_products->EOF) {
            if($bundle_products->fields['products_is_osds']=="Y") {
                
                if ($bundle_products->fields['products_price'] == '') {
				    $bundle_price = (zen_get_products_base_price($bundle_products->fields['products_id'],true,unserialize($bundle_products->fields['options'])) * (int)$bundle_products->fields['products_quantity']);
			     } else {
				    $bundle_price = $bundle_products->fields['products_price'];
			}
                ...
                $curl_dap_data = array(
            				...
            				'address_zip' => $this->customer['postcode'],//'110018',
            				'address_country' => $this->customer['country']['title'],//'India',
            				'mc_gross' => $bundle_price,//'499',
            				'mc_currency' => 'INR',
            				'payment_status' => 'Completed',
            				'item_name' => $bundle_products->fields['products_osds_name'],//$this->products[$i]['products_osds_name'],//'NSO Level 2 - Class 3',
            				'secret_key' => 'mtgbooks'
            			);
            			$http_code=curl_dap($curl_dap_data);
            			if($http_code!=200){
            ...
            $bundle_products->MoveNext();
        }
      }*/
      // added by Taras to check products in bundle for OSDS EOF
      
/*Rajeev GreyBox, OSDS automation Ends*/
...
// zenbundles: 4 of 4 start
	  ...
	  $this->notify('NOTIFY_ORDER_DURING_CREATE_ADDED_PRODUCT_LINE_ITEM', array_merge(array('orders_products_id' => $order_products_id,'i'=>$i), $sql_data_array));
// zenbundles: 4 of 4 end
...
/*Added by Rajeev to check if this is purely an e-order*/

  function check_if_skip_order() {
	  global $db;
	  $only_eproducts=false;	
	  for ($i = 0, $exclude_this_from_printing_count = 0, $n = sizeof($this->products); $i < $n; $i++) {
		$exclude_this_from_printing = false;
		if(!is_osds_product($this->products[$i]['id'])) {//If it is not an OSDS product then check if it is a downloadable product using attributes
			if (isset($this->products[$i]['attributes']) && (sizeof($this->products[$i]['attributes']) > 0)) {
			  for ($j=0, $k=sizeof($this->products[$i]['attributes']); $j<$k; $j++) {
				//if($this->products[$i]['attributes'][$j]['option'] == 'Available Formats' &&  $this->products[$i]['attributes'][$j]['value'] == 'PDF') {
				if($this->products[$i]['attributes'][$j]['option'] == 'Available Formats') {
					$exclude_this_from_printing = true;
				}
			  }
			} else {//Check if it is marked Digital
			    $skip_packing_slip_sql = "SELECT products_digital
										  FROM ".TABLE_PRODUCTS."
										  WHERE products_id = ".(int)$this->products[$i]['id'];
				$skip_packing_slip = $db->execute($skip_packing_slip_sql);
				if ($skip_packing_slip->fields['products_digital'] == 'Y') $exclude_this_from_printing = true;
			}
		} else {//This is an OSDS product, so exclude it.
			$exclude_this_from_printing = true;
		}

		if($exclude_this_from_printing) {
			$exclude_this_from_printing_count++;
		}
	  }

	  if($n==$exclude_this_from_printing_count)
		return true;
	  else
		return false;
  }
